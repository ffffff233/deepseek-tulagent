...
产品、仓库、pip 分发、桌面端和 CLI 均统一使用 **DeepSeekFathom**。已有用户数据会自动迁移，且不会覆盖原有会话或设置。

## 功能

- DeepSeek 优先配置：`DEEPSEEK_API_KEY`、`DEEPSEEK_BASE_URL`、`DEEPSEEK_MODEL`
- 默认全局命令：`deepseekfathom`
- DeepSeek V4 模型别名：`pro`、`v4-pro`、`flash`、`v4-flash`，上下文预算按 100 万 token 处理
...
- 五档思考模式：`fast`、`balanced`、`deep`、`ultra`、`max`，其中 `max` 最强
- 本地技能目录：自动发现 `SKILL.md`
- 扩展目录：用户插件保存在 `~/.deepseekfathom/plugins`，用户技能、插件、配置和会话不会被桌面端更新覆盖
- 会话保存和恢复：JSONL 格式
...
工具会把 Windows 风格路径映射到当前工作区内，避免误写到未知位置。要指定工作区可以先设置 `DEEPSEEKFATHOM_WORKSPACE`。
...
dist\installer\DeepSeekFathom-0.1.17-Setup.exe
```
...
用户 MCP 配置保存在 `~/.deepseekfathom/config.json`，覆盖安装会保留。项目和插件提供的 MCP 会在同一页显示，但由各自来源管理，不能从用户列表误删。
...
| `DEEPSEEKFATHOM_WORKSPACE` | 当前目录 | 工作目录 |
| `DEEPSEEKFATHOM_MAX_TOOL_ROUNDS` | `256` | 最大工具循环次数 |
| `DEEPSEEKFATHOM_MAX_TOKENS` | `8192` | 最大输出 token |
| `DEEPSEEKFATHOM_REQUEST_TIMEOUT` | `180` | 请求超时时间 |
| `DEEPSEEKFATHOM_SEARCH_ENGINES` | `baidu,bing,duckduckgo` | `web_search` 使用的第三方搜索引擎顺序；结果页抓取默认关闭，开启后会检查 robots.txt |
...
五档都会向 DeepSeek API 发送真实 `thinking` 开关和表中的 `reasoning_effort`，其中 `max` 会原样传给上游。Anthropic 和 Gemini 会换算为各自的原生预算参数。`balanced` 及以上还会在客户端进行真实内部思考轮次：先调用模型生成私有规划，再把规划作为本轮回答上下文使用。DeepSeek V4 的上下文预算按 100 万 token 窗口处理。
...
- `~/.deepseekfathom/config.json`，包括 API key、base URL、默认模型
- `~/.deepseekfathom/skills` 和工作区技能目录
- 会话目录
...
- 不要提交 `~/.deepseekfathom/config.json`，里面可能有 API key。
