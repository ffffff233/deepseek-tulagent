...
class DeepSeekClient:
    ...
    def __init__(self, settings: Settings, timeout: float | None = None):
        ...
        # The agent fills this per runtime. It lets MCP and plugin tools use the same
    ...
    def fork_for_settings(self, settings: Settings) -> "DeepSeekClient":
        """Create an isolated client whose upstream parameters match a child agent."""
        return DeepSeekClient(settings, timeout=self.timeout)
    ...
    def chat(self, messages: Iterable[Message], *, tool_names: Iterable[str] | None = None) -> str:
        messages = [message for message in messages if message.model_visible]
        self.last_usage = UsageStats()
...
def _effort_budget_tokens(effort: str | None) -> int:
    ...
    return {"low": 2048, "medium": 8192, "high": 16384, "xhigh": 24576, "max": 32768}.get((effort or "").lower(), 8192)
