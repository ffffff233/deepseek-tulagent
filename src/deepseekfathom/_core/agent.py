...
SYSTEM_PROMPT = r"""You are DeepSeekFathom, a concise coding agent running in a local workspace.
...
- git_status(timeout?)
- git_diff(scope?, path?, context?, max_chars?, timeout?): inspect working, staged, or latest-commit changes without modifying Git state
...
- stop_service(name)
- service_status(name)
- configure_mcp_server(name, transport?, url?, command?, args?, env?, cwd?, headers?, enabled?, originalName?, replace?): persist a user-owned MCP server only when the user explicitly asks to configure or save it
- list_mcp_servers(): list configured MCP servers and connection state without exposing secret values
...
- Treat MCP connection and MCP configuration as different intents. The desktop `/mcp` command only connects already configured servers. Call configure_mcp_server only when the user explicitly asks to configure, save, install, or update an MCP server; never use it merely because a server is being connected.
- Never repeat MCP header or environment secret values in prose, tool summaries, or approval text.
...
class FathomAgent:
    ...
    def run(
        ...
        *,
        ...
        new_session = not any(message.model_visible for message in session.messages)
        hook_context = ""
        ...
        for rounds in range(1, round_limit + 1):
            ...
            if not tool_calls:
                ...
                session.append(Message("assistant", assistant_text, turn_id=turn_id))
                if goal and not goal_answer_is_terminal(assistant_text):
                    session.append(Message("user", goal_continuation_prompt(goal), turn_id=turn_id))
                    continue
            ...
            session.append(Message("assistant", assistant_record, turn_id=turn_id))
            last_turn_had_tool_result = False
            last_turn_had_tool_error = False
            ...
            for name, arguments in tool_calls:
                ...
                if on_event:
                    on_event(f"tool {name} {summarize_arguments(arguments, tool_name=name)}")
                ...
                session.append(Message(
                    "user",
                    tool_result_message(name, trim_tool_content(content)),
                    images=tool_images,
                    turn_id=turn_id,
                ))
            ...
            if self.hook_runner is not None:
                ...
                self._emit_hook_report(stop_report, on_event)
            if final_answer:
                session.append(Message("assistant", final_answer, turn_id=turn_id))
            else:
    ...
    def _needs_confirmation(self, name: str) -> bool:
        ...
        contract = self.tool_contracts.get(name)
        if contract is not None and contract.always_confirm:
            return True
    ...
    def _run_one_subagent(
        ...
        *,
        ...
        resolved_thinking = ThinkingMode.resolve(thinking)
        sub_settings = self.settings.with_runtime(
            max_tokens=resolved_thinking.max_tokens,
            thinking_enabled=resolved_thinking.api_thinking,
            reasoning_effort=resolved_thinking.reasoning_effort,
        )
        fork_client = getattr(self.client, "fork_for_settings", None)
        sub_client = fork_client(sub_settings) if callable(fork_client) else self.client
        owns_sub_client = sub_client is not self.client
        ...
        try:
            ...
            if owns_sub_client:
                parent_usage = getattr(self.client, "usage", None)
                child_usage = getattr(sub_client, "usage", None)
                if parent_usage is not None and child_usage is not None and hasattr(parent_usage, "merge"):
                    parent_usage.merge(child_usage)
                close = getattr(sub_client, "close", None)
                if callable(close):
                    close()
        if self.hook_runner is not None:
    ...
    def _approved(self, name: str, arguments: dict[str, Any]) -> bool:
        contract = self.tool_contracts.get(name)
        if contract is not None and contract.always_confirm:
            return bool(self.approve and self.approve(name, arguments))
    ...
    def _with_internal_thinking(self, messages: list[Message], on_event: Callable[[str], None] | None = None) -> list[Message]:
        ...
        # which then leaked into the transcript. Set DEEPSEEKFATHOM_LOCAL_DELIBERATION=1 to
        # re-enable the old behavior.
        if os.getenv("DEEPSEEKFATHOM_LOCAL_DELIBERATION", os.getenv("DSTUL_LOCAL_DELIBERATION", "")) != "1":
            return messages
...
def parse_tool_calls(text: str) -> list[tuple[str, dict[str, Any]]]:
    ...
    if os.getenv("DEEPSEEKFATHOM_PARSE_ACTION_SHELL", os.getenv("DSTUL_PARSE_ACTION_SHELL", "0")).lower() in {"1", "true", "yes"}:
...
def filter_internal_automation_messages(messages: list[Message]) -> list[Message]:
    filtered = [
        ...
        if message.model_visible
        and not (message.role == "user" and is_internal_automation_prompt(message.content))
    ]
...
def claims_local_action_completed(text: str) -> bool:
    ...
    cues = (
        ...
        "started", "stopped", "uploaded", "已配置", "配置完成", "配置成功", "configured",
    )
...
def compact_context_messages(
    ...
    *,
    ...
    if not force and os.getenv("DEEPSEEKFATHOM_AUTO_COMPACT", os.getenv("DSTUL_AUTO_COMPACT", "1")).lower() in {"0", "false", "no"}:
        return messages
...
def summarize_arguments(arguments: dict[str, Any], *, tool_name: str = "") -> str:
    sensitive_fragments = ("token", "secret", "password", "passwd", "authorization", "cookie", "api_key", "apikey")

    def summarize_value(key: str, value: Any) -> str:
        normalized = key.casefold().replace("-", "_")
        if any(fragment in normalized for fragment in sensitive_fragments):
            return "<hidden>"
        if normalized in {"headers", "env", "environment"} and isinstance(value, dict):
            return "keys:[" + ",".join(sorted(str(item) for item in value)) + "]"
        if isinstance(value, dict):
            safe: dict[str, Any] = {}
            for nested_key, nested_value in value.items():
                nested = str(nested_key)
                nested_normalized = nested.casefold().replace("-", "_")
                safe[nested] = "<hidden>" if any(
                    fragment in nested_normalized for fragment in sensitive_fragments
                ) else nested_value
            return str(safe)
        return str(value)
    ...
    for key, value in arguments.items():
        text = summarize_value(str(key), value).replace("\n", "\\n")
        if len(text) > 80:
            text = text[:77] + "..."
