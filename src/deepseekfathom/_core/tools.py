...
TOOL_DESCRIPTIONS = {
    ...
    "git_diff": "read: inspect working tree, staged, or latest commit diff without modifying Git state",
...
class ToolRegistry:
    ...
    def git_diff(self, arguments: dict[str, Any]) -> ToolResult:
        scope = str(arguments.get("scope") or "working").strip().lower()
        if scope not in {"working", "staged", "last_commit"}:
            raise ToolError("git_diff scope must be working, staged, or last_commit")
        context = max(0, min(20, int(arguments.get("context", 5))))
        max_chars = max(2_000, min(500_000, int(arguments.get("max_chars", 120_000))))
        timeout = max(1, min(120, int(arguments.get("timeout", 30))))
        argv = ["git"]
        if scope == "last_commit":
            argv.extend(["show", "--format=fuller", "--no-ext-diff", f"--unified={context}", "HEAD"])
        else:
            argv.extend(["diff", "--no-ext-diff", f"--unified={context}"])
            if scope == "staged":
                argv.append("--cached")
        ...
        if raw_path:
            ...
            try:
                ...
                raise ToolError("git_diff path must be inside the workspace") from exc
        ...
        if completed.returncode != 0:
            return ToolResult(False, output.strip() or f"git diff exited with {completed.returncode}")
        bounded, truncated, omitted = _bounded_diff(output, max_chars=max_chars)
        if truncated:
            bounded += f"\n\n[diff truncated: {omitted} lines omitted]"
        return ToolResult(True, bounded.strip() or "clean")
    ...
    def clone_repo(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        if github_repo:
            ...
            with tempfile.TemporaryDirectory(prefix="fathom-clone-") as tmp:
                tmp_path = Path(tmp)
                for ref in refs:
    ...
    def start_service(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        name = require_str(arguments, "name")
        command = require_str(arguments, "command")
        services_dir = self.workspace / ".deepseekfathom" / "services"
    ...
    def stop_service(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        name = require_str(arguments, "name")
        pid_path = self.workspace / ".deepseekfathom" / "services" / f"{safe_name(name)}.pid"
...
def download_archive(url: str, dest: Path, timeout: int) -> tuple[bool, str]:
    request = urllib.request.Request(url, headers={"User-Agent": "DeepSeekFathom/0.1"})
    try:
...
def extract_repo_archive(archive_path: Path, dest: Path) -> tuple[bool, str]:
    try:
        with tempfile.TemporaryDirectory(prefix="fathom-extract-") as tmp:
            tmp_path = Path(tmp)
...
def fetch_text_url(url: str, *, timeout: int, max_bytes: int = 1_500_000) -> str:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "(KHTML, like Gecko) Chrome/126.0 Safari/537.36 DeepSeekFathom/0.1",
...
def can_fetch_url(url: str, *, timeout: int) -> tuple[bool, str]:
    ...
    allowed = parser.can_fetch("DeepSeekFathom", url)
...
def local_search_url_candidates(search_url: str | None = None) -> list[str]:
    configured = string_or_none(search_url) or string_or_none(
        os.getenv("DEEPSEEKFATHOM_SEARCH_URL", os.getenv("DSTUL_SEARCH_URL"))
    )
    if configured:
...
def web_search_engine_order(options: dict[str, str | None] | None = None) -> list[str]:
    raw = string_or_none((options or {}).get("engines")) or string_or_none(
        os.getenv("DEEPSEEKFATHOM_SEARCH_ENGINES", os.getenv("DSTUL_SEARCH_ENGINES"))
    )
