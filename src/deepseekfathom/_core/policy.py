...
class ThinkingMode:
    ...
    def resolve(cls, mode: str) -> "ThinkingMode":
        modes = {
            ...
            "max": cls("max", "deepseek-v4-pro", 384000, "Use maximum practical reasoning for ambiguous multi-step engineering work and verify every important conclusion.", 6, True, "max"),
        }
        try:
            return modes[mode]
        except KeyError as exc:
    ...
    def names(cls) -> list[str]:
        return ["auto", "off", "instant", "fast", "standard", "balanced", "careful", "deep", "deeper", "ultra", "max"]

    @classmethod
    def user_selectable_names(cls) -> list[str]:
        return ["fast", "balanced", "deep", "ultra", "max"]
