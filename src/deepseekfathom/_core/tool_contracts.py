...
class ToolContract:
    ...
    # Some extension operations are security boundaries even in full-access mode.
    # For example, persisting an stdio MCP server can authorize an arbitrary local
    # executable to start on future launches, so an explicit user confirmation is
    # always required rather than being silently inherited from root/yolo mode.
...
def coerce_tool_contract(value: Any, *, handler: ToolHandler | None = None) -> ToolContract:
    if isinstance(value, ToolContract):
        return value if handler is None else ToolContract(
            ...
            always_confirm=value.always_confirm,
        )
    if isinstance(value, Mapping):
        ...
        always_confirm = bool(value.get("always_confirm", value.get("alwaysConfirm", False)))
    else:
        ...
        always_confirm = bool(getattr(value, "always_confirm", False))
    if not name:
