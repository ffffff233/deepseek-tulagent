...
class UserMCPServerNotFoundError(UserMCPConfigError):
    """The user MCP configuration is valid, but the requested entry is absent."""
...
def get_user_mcp_server(name: str, home: Path | None = None) -> dict[str, Any]:
    ...
    with _CONFIG_LOCK:
        ...
        if clean_name not in servers:
            raise UserMCPServerNotFoundError(f'找不到用户 MCP 服务 "{clean_name}"')
    ...
    *,
    ...
    with _CONFIG_LOCK:
        ...
        if old_name is not None and old_name not in servers:
            raise UserMCPServerNotFoundError(f'找不到要重命名的用户 MCP 服务 "{old_name}"')
